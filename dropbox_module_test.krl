...
Test the Dropbox module
    ...
    key dropbox {
       "app_key" : "<insert Dropbox app key>",
       "app_secret" : "<insert Dropbox app secret>"
    }      
    
    use module a169x701 alias CloudRain
    use module b16x0 alias dropbox 
          with app_key = keys:dropbox('app_key') and
  ...
  global {
    ...
    my_tokens = function() {{'access_token' : ent:access_token,
  ...
  rule get_request_token { 
    ...
    if(not authorized) then {
      dropbox:get_request_token(my_tokens());	   
    }  
  }
  ...
  rule process_request_token {
    ...
    pre {
      ...
      url = dropbox:generate_authorization_url(tokens{'oauth_token'});
      my_html = <<
...
>>;
  ...
  rule get_access_token {
    ...
    if(not authorized) then {
      dropbox:get_access_token(my_tokens().put({'request_token': ent:request_token, 
                                              'request_token_secret': ent:request_token_secret}));
    }    
  }
